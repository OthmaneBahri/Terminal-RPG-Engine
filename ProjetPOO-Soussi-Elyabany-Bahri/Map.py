...
class Map(object):
    ...
    def move(self, e, way):
        ...
        print(e.invisibilite)
        if e.invisibilite > 0:
            e.invisibilite = e.invisibilite-1
        orig = self.pos(e)
        dest = orig + way
        if dest in self:
    ...
    def moveAllMonsters(self):
        ...
        if self._hero.repos==1 and self._hero.limRepos <= 3:
         self._hero.limRepos+=1
         self._hero.hp+=5
         print('ZZZZZZZZ')
         for i in range(5):
           ...
           for e in self._elem:
            ...
            if isinstance(e, Creature) and e != self._hero and c.distance(h) < 6:
                if e.abbrv == 'I' : #Rend le marchand immobile
                    return True
                d = c.direction(h)
                if e.sante==1:
                    e.hp=e.hp-1
                if self.get(c + d) in [Map.ground, self._hero]:
                    self.move(e, d)
                if e.abbrv=='R': #Donne 2 actions au Rapido 
                    h = self.pos(self._hero)
                    for w in self._elem:
                      c = self.pos(w)
                      if isinstance(w, Creature) and w != self._hero and c.distance(h) < 6:
                         d = c.direction(h)
                         if self.get(c + d) in [Map.ground, self._hero]:
                            self.move(w, d)
        self._hero.repos=0
        ...
        if self._hero.invisibilite >= 1: #Rend le héros invisible 
            return True
        for e in self._elem:
            c = self.pos(e)
            if isinstance(e, Creature) and e != self._hero and c.distance(h) < 6:
